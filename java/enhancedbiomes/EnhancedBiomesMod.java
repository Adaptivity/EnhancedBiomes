...
public class EnhancedBiomesMod
{
		...
		if(useNewStone != 1) return typeGeoDefault;
			...
			switch(getCatForBiome(biome)) {
				case GRASS:
					return typeGeoGrass;
				case CANYONS:
					return typeGeoSandCanyons;
				case SAND:
					return typeGeoSandCanyons;
				case BROADLEAF:
					return typeGeoWoodland;
				case BOREAL:
					return typeGeoWoodland;
				case WETLAND:
					return typeGeoWetland;
				case MOUNTAINS:
					return typeGeoMountains;
				case VOLCANO:
					return typeGeoVolcanoSea;
				case SEA:
					return typeGeoVolcanoSea;
				case ICE:
					return typeGeoIce;
	...
	public static double getStoneNoiseForCoords(int oriX, int oriZ) {
		int x = Math.abs(oriX), z = Math.abs(oriZ);
		return (oriX >= 0 ? oriZ >= 0 ? stoneNoisePP : stoneNoisePN : oriZ >= 0 ? stoneNoiseNP : stoneNoiseNN).size() < x || (oriX >= 0 ? oriZ >= 0 ? stoneNoisePP : stoneNoisePN : oriZ >= 0 ? stoneNoiseNP : stoneNoiseNN).get(x).size() < z ? 0 : (oriX >= 0 ? oriZ >= 0 ? stoneNoisePP : stoneNoisePN : oriZ >= 0 ? stoneNoiseNP : stoneNoiseNN).get(x).get(z);
	}
