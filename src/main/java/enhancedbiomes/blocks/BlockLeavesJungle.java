...
public class BlockLeavesJungle extends BlockLeavesBase implements IShearable
{
	public static final String[] LEAF_TYPES = BlockLogJungle.woodType;
	private IIcon[][] iconArray = new IIcon[2][16];
	int[] adjacentTreeBlocks;

	public BlockLeavesJungle() {
	...
	public int getRenderColor(int par1) {
		return par1 % 4 != 1 ? par1 % 4 != 2 ? ColorizerFoliage.getFoliageColorBasic() : 0xF054DB : 0xFFFFFF;
	}
	...
	public int getDamageValue(World p_149643_1_, int p_149643_2_, int p_149643_3_, int p_149643_4_) {
		return this.damageDropped(p_149643_1_.getBlockMetadata(p_149643_2_, p_149643_3_, p_149643_4_)) % 4;
	}
	...
	public int colorMultiplier(IBlockAccess par1IBlockAccess, int par2, int par3, int par4) {
		int a = par1IBlockAccess.getBlockMetadata(par2, par3, par4);
		...
		if(a % 4 == 1) {
			...
			for(int k1 = -1; k1 <= 1; ++k1) {
				for(int l1 = -1; l1 <= 1; ++l1) {
					int i2 = par1IBlockAccess.getBiomeGenForCoords(par2 + l1, par4 + k1).getBiomeFoliageColor(par2 + l1, par3, par4 + k1);
					l += (i2 & 16711680) >> 16;
					i1 += (i2 & 65280) >> 8;
					j1 += i2 & 255;
				}
	...
	/**
	 * Called on server worlds only when the block has been replaced by a different block ID, or the same block with a different metadata value, but before the new metadata value is set. Args: World,
	 * x, y, z, old block ID, old metadata
	 */
	@Override
	...
	public Item getItemDropped(int par1, Random par2Random, int par3) {
		return Item.getItemFromBlock(EnhancedBiomesBlocks.saplingEB);
	}
	...
	public IIcon getIcon(int par1, int par2) {
		return this.iconArray[Blocks.leaves.isOpaqueCube() ? 1 : 0][par2];
	}
	...
	public void registerBlockIcons(IIconRegister par1IconRegister) {
		for(int i = 0; i < this.iconArray.length; ++i) {
			for(int i2 = 0; i2 < this.iconArray[i].length; ++i2) {
				if(LEAF_TYPES[i2 % 4] != "") {
					this.iconArray[i][i2] = par1IconRegister.registerIcon("enhancedbiomes:" + this.getTextureName() + "_" + LEAF_TYPES[i2 % 4] + (i == 1 ? "_opaque" : ""));
				}
			}
		}
	}
